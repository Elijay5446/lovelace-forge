...
The exact setup is:
STEP 1 — Download the Forge Bridge .zip from the page and unzip it. Copy the file "LovelaceForgeBridge.cs" into your Unity project under "Assets/Editor/" (create the Editor folder if it doesn't exist). Unity compiles it automatically. Then open the menu "Tools ▸ Lovelace Forge ▸ Start Bridge". The Unity Console prints "🟢 Listening on http://localhost:9876".
STEP 2 — Start a Cloudflare tunnel so Lovelace can reach your editor. Install cloudflared (Windows: "winget install --id Cloudflare.cloudflared"; macOS: "brew install cloudflared"), then run "cloudflared tunnel --url http://localhost:9876". Copy the "https://...trycloudflare.com" URL it prints — that is your tunnel URL.
...
- "Bridge won't start / port in use": make sure nothing else uses port 9876; in the menu click Tools ▸ Lovelace Forge ▸ Start Bridge; on Windows run (as admin) "netsh http add urlacl url=http://localhost:9876/ user=Everyone".
- "Lovelace says it can't reach the URL": confirm the bridge is running (visit http://localhost:9876/health in a browser), confirm cloudflared is still running, and re-copy the fresh tunnel URL.
